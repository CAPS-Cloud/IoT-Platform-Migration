    ...
    add(req, res) {
        Devices.findById(req.params.id).then(device => {
            if (!device) {
                ...
                return res.status(400).json({ name: 'MappingIsRequired', errors: [{ message: 'Please enter mapping' }] });
            } else if (!this._isJsonString(req.body.mapping)) {
                return res.status(400).json({ name: 'MappingIsNotValidJSON', errors: [{ message: 'Mapping is not valid JSON' }] });
            } else {
                if (!!req.file) {
                    Sensors.create({ name: req.body.name, description: req.body.description, deviceId: device.id }).then(sensor => {
