...
const controller = new class {
    ...
    get(req, res, next) {
        ...
        if (typeof bearerHeader !== 'undefined') {
            ...
            jwt.verify(bearerToken, CONSUMER_SECRET, { algorithm: 'RS256' }, (err, authData) => {
                if (!err) {
                    if (authData.iis == "iotplatform") {
                        Sensors.findOne({ where: { id: { [Op.eq]: req.params.sensor_id } }, include: [{ model: Consumers }, { model: Devices }] }).then(sensor => {
                            if (sensor) {
                                if (sensor.consumers && sensor.consumers.some(consumer => consumer.id == authData.sub)) {
