...
testapp.post('/', (req, res) => {
  ...
  if (typeof bearerHeader !== 'undefined') {
    ...
    if (bearer.length != 2) {
      if (role.localeCompare('SIGNIN') == 0) {
        // deferred.resolve({
        //   statusCode: 200,
        ...
        // });
        res.status(200)
        res.send("Bearer Length not equal to 2")
      } else {
        // deferred.reject({
        //   statusCode: 401,
        //   headers: { "Content-Type": "application/json" },
        ...
        // })
        res.status(401)
        res.send("Error Bearer Length not equal to 2")
      }
    } else {
        ...
        jwt.verify(
          ...
          (err, authData) => {
            if (!err) {
                ...
                Users.findOne({ where: { id: { [Op.eq]: authData.sub } } }).then(
                  (data) => {
                    if (data) {
                      ...
                      // });
                      res.status(401)
                      res.send("User data not available")
                    }
                  }
                );
              }
              ...
              // });
              res.status(401)
              res.send("Error in jwt verification")
            }
        }
      );
    }
    ...
    // });
    res.status(401)
    res.send("Error bearerHeader undefined")
  }
})
