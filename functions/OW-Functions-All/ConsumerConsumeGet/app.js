...
const controller = new (class {
    get (req) {
        ...
        const bearerHeader = req.rawHeaders[authIndex + 1]
                    ...
                    if (!err) {
                        Sensors.findOne({
                            ...
                            if (sensor) {
                                ...
                                // console.log("sensor.device.userId.toString():", sensor.device.userId.toString())
                                    ...
                                    if (req.url.indexOf('scroll') !== -1 && req.body && ("scroll_id" in req.body)){
                                        ...
                                        if (req.body){
                                            ...
                                            deferred.reject({
                                                statusCode: 401,
                                                headers: { "Content-Type": "application/json" },
                                                body: { error: "Does not contain body" }
                                              });
                                        }
                                ...
                                deferred.reject({
                                    statusCode: 401,
                                    headers: { "Content-Type": "application/json" },
                                    body: { error: "Sensor does not exist" }
                                  });
                            }
                        }).catch(err => {
                            deferred.reject({
